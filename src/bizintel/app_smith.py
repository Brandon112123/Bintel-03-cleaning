"""app_brandon.py - modified business intelligence project.
...
Process:
    ...
    - Visualize sales by region, product category, and payment method.
...
uv run python -m bizintel.app_brandon

MODIFICATION:
  This copy adds a new analysis and bar chart showing total sales by payment method.
  The original example file should remain unchanged.
"""
...
# === Section 2.3 DEFINE A SALES BY PAYMENT METHOD FUNCTION ===

# Define a reusable function that takes the sales DataFrame as input
# and returns a DataFrame with total sales by payment method.
# This technical modification adds a new business insight and visualization.


def sales_by_payment_method(df_sales: pd.DataFrame) -> pd.DataFrame:
    """Aggregate total sales amount by payment method.

    WHY: Payment method is an important business dimension.
    Understanding how customers pay can support decisions about
    payment processing, customer convenience, and sales strategy.

    Args:
        df_sales: Sales DataFrame with PaymentMethod and SaleAmount columns.

    Returns:
        DataFrame with PaymentMethod and SaleAmount columns,
        sorted by SaleAmount.
    """
    LOG.info("Aggregating sales by payment method")

    # Make a copy of the sales DataFrame to avoid modifying the original.
    df_sales = df_sales.copy()

    # Convert SaleAmount to numeric values.
    # Any invalid values are converted to NaN ("not a number").
    df_sales["SaleAmount"] = pd.to_numeric(
        df_sales["SaleAmount"],
        errors="coerce",
    )

    # Clean the PaymentMethod values by removing extra spaces
    # and capitalizing each word for consistent formatting.
    df_sales["PaymentMethod"] = (
        df_sales["PaymentMethod"].astype("string").str.strip().str.title()
    )

    # Remove rows that do not have a valid payment method or sale amount.
    df_sales = df_sales.dropna(subset=["PaymentMethod", "SaleAmount"])

    # Group sales by PaymentMethod and add the SaleAmount values.
    grouped: pd.Series = pd.Series(
        df_sales.groupby("PaymentMethod")["SaleAmount"].sum()
    )

    # Convert the grouped Series back into a DataFrame
    # and sort it from the highest total sales to the lowest.
    df_payment: pd.DataFrame = grouped.reset_index().sort_values(
        "SaleAmount",
        ascending=False,
    )

    # Log the leading payment method when results are available.
    if not df_payment.empty:
        top_payment_method: str = str(df_payment.iloc[0]["PaymentMethod"])
        top_sales: float = float(df_payment.iloc[0]["SaleAmount"])
        LOG.info(f"  Top payment method: {top_payment_method} (${top_sales:,.2f})")
    else:
        LOG.warning("No valid payment-method sales data was found.")

    LOG.info("Returning DataFrame with total sales by payment method")
    return df_payment
...
def main() -> None:
    ...
    LOG.info("CALL a function to get sales by payment method........")
    df_payment = sales_by_payment_method(df_sales)

    LOG.info("CALL a function to plot sales by payment method........")
    plot_bar(
        df=df_payment,
        x="PaymentMethod",
        y="SaleAmount",
